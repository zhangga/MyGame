...
public final class SerialKeyExecutor<K> implements Executor
{
	...
	public final class Serial
	{
		...
		public Serial()
		{
			header = new Serial.Task<>(null, dummyHeader);
			reset();
		}
		...
		public <T> Future<T> submit(K key, Callable<T> callable)
		{
			lock.lock();
			try {
				Serial.Task<T> lastTask = addLast(new Serial.Task<>(key, callable));
				if (size == 1)
				{
					try {
		...
		public void scheduleNext(Serial.Task<?> done)
		{
		...
		private Serial.Task<?> removeFirst()
		{
			return remove(getFirst());
		}
		
		private Serial.Task<?> remove(Task<?> task)
		{
